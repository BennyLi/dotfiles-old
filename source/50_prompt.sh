...
function prompt_git() {
  prompt_getcolors
  local status output flags branch upstream
  status="$(git status 2>/dev/null)"
  [[ $? != 0 ]] && return;
  output="$(echo "$status" | awk '/# Initial commit/ {print "(init)"}')"
  [[ "$output" ]] || output="$(echo "$status" | awk '/# On branch/ {print $4}')"
  [[ "$output" ]] || output="$(git branch | perl -ne '/^\* \(detached from (.*)\)$/ ? print "($1)" : /^\* (.*)/ && print $1')"
  upstream="$(git branch -vv | sed -n 's/^\* .*\[\([a-zA-Z0-9\/_]*\).*$/\1/p')"
  output="$output$c1:$c5$upstream"
  flags="$(
...
function prompt_svn() {
  ...
  if [[ "$info" ]]; then
    ...
    echo "$c1[$c4$last$c1:$c4$current$c1]$c9"
  fi
}
